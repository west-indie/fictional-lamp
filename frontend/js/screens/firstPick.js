// frontend/js/screens/firstPick.js

import { GameState, changeScreen } from "../game.js";
import { Input } from "../ui.js";
import { playUIConfirmBlip, playUIBackBlip } from "../sfx/uiSfx.js";
import { SCREEN, FIRST_PICK_LAYOUT as L } from "../layout.js";

import { drawCenteredGenres } from "../ui/genreStyle.js";
import { computeFirstPickEffect, describeFirstPick } from "../systems/onefourEffectSystem.js";
import { ImageCache, computeCoverCrop } from "../core/ImageCache.js";

const FIRST_HEADLINES = [
  "Group Leader Takes the Reins!",
  "The Captain Steps Forward!",
  "The Lead Takes Charge!",
  "Opening Act Dominance!",
  "The Spark Ignites the Team!",
  "First Pick Sets the Tone!"
];
...
function movieLabel(movie) {
  return String(movie?.shortTitle || movie?.title || "Movie 1").trim();
}
...
// --- Non-stat templates (unchanged wording)
const NONSTAT_TEMPLATES = {
  COMEDY: {
    leader: (name, pct) => `${name} has a ${pct}% chance to refund the last Special’s cooldown!`,
    team: (pct) => `Specials have a ${pct}% chance to refund the last cooldown!`
  },
  THRILLER: {
    leader: (name, pct) => `${name} has a ${pct}% chance to trigger a critical follow-up!`,
    team: (pct) => `Basic attacks have a ${pct}% chance to trigger a critical follow-up!`
  },
  MYSTERY: {
    leader: (name, pct) => `${name}’s items get +${pct}% stronger random stat rolls!`,
    team: (pct) => `Items get +${pct}% stronger random stat rolls!`
  },
  ADVENTURE: {
    leader: (name) => `${name}’s items are stronger!`,
    team: () => `Items are stronger!`
  },
  FANTASY: {
    leader: (name) => `${name}’s Special Moves hit harder!`,
    team: () => `Special Moves hit harder!`
  },
  MUSICAL: {
    leader: (name) => `${name}’s Special Moves gain a bonus in a sequence!`,
    team: () => `Special Moves gain a bonus in a sequence!`
  },
  SCIFI: {
    leader: (name) => `${name} builds reuse stacks that boost repeated Specials!`,
    team: () => `Special Moves build reuse stacks that boost repeats!`
  }
};
...
function formatStatSentence({ whoLabel, statKey, pct }) {
  ...
  return `${whoLabel} receives an extra +${pct}% on ${phrase}!`;
}

function resolveEffectLine({ genre, audience, describedLine, nonStat, movie }) {
  const pct = parsePctFromDescribeLine(describedLine);
  const statKey = parseStatKeyFromDescribeLine(describedLine);

  if (pct != null && statKey) {
    const whoLabel = audience === "LEADER" ? movieLabel(movie) : "Team";
    return formatStatSentence({ whoLabel, statKey, pct });
  }
  ...
  if (!describedLine || raw.includes("no stat change")) {
    ...
    const name = movieLabel(movie);

    if (genre === "COMEDY" && nonStat?.chance != null) {
      const p = pctInt(nonStat.chance);
      return audience === "LEADER" ? tmpl.leader(name, p) : tmpl.team(p);
    }

    if (genre === "THRILLER" && nonStat?.chance != null) {
      const p = pctInt(nonStat.chance);
      return audience === "LEADER" ? tmpl.leader(name, p) : tmpl.team(p);
    }

    if (genre === "MYSTERY" && nonStat?.rollBonus != null) {
      const p = pctInt(nonStat.rollBonus);
      return audience === "LEADER" ? tmpl.leader(name, p) : tmpl.team(p);
    }

    if (audience === "LEADER") return typeof tmpl.leader === "function" ? tmpl.leader(name) : "";
    return typeof tmpl.team === "function" ? tmpl.team() : "";
  }

  const cleaned = String(describedLine)
    .replace(/^Self:\s*/i, "")
    .replace(/^Team:\s*/i, "")
    .trim();
...
export const FirstPickScreen = {
  update(mouse) {
    ensureCampaignBuckets();

    const movie = GameState.party?.movies?.[0] || null;

    if (!GameState.campaign.flavor.firstHeadline) {
      GameState.campaign.flavor.firstHeadline = pick(FIRST_HEADLINES);
    }

    if (!GameState.campaign.effects.first) {
      const eff = computeFirstPickEffect(movie, GameState.campaign.flavor.firstHeadline);
      GameState.campaign.effects.first = eff;
      GameState.campaign.firstPickApplied = eff?.movieId || null;
    }
  ...
  render(ctx) {
    ...
    const movie = GameState.party?.movies?.[0] || null;
    const eff = GameState.campaign?.effects?.first || null;
    ...
    const [rawSelfLine, rawTeamLine] = describeFirstPick(eff);

    const selfLine = resolveEffectLine({
      genre: g1,
      audience: "LEADER",
      describedLine: rawSelfLine,
      nonStat: eff?.selfNonStat || null,
      movie
    });

    const teamLine = resolveEffectLine({
      genre: g2,
      audience: "TEAM",
      describedLine: rawTeamLine,
      nonStat: eff?.teamNonStat || null,
      movie
    });
    ...
    drawCenteredSingleLineFit(
      ctx,
      eff?.headline || "Group Leader Takes the Reins!",
      textCenterX,
      L.headline.y,
      L.headline.maxW,
      L.headline.startPx,
    ...
    const linesUsed1 = drawCenteredWrappedFit(
      ctx,
      selfLine,
      textCenterX,
      L.effects.y,
    ...
    const nextY = L.effects.y + linesUsed1 * L.effects.lineStep + L.effects.gapAfterBlock;
